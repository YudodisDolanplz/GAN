...
def train_D_on_generated() :
    ...
    fake_error = criterion( fake_decision, torch.zeros(seq_len, d_minibatch_size, d_input_size))  # zeros = fake
    fake_error.backward()
    #print(fake_error)
...
def train_G(): #function gives back generated fake data
    noise = torch.randn(seq_len, g_minibatch_size, input_size) #generating: noise = random value vektor
    fake_data = G(noise) #run noise through RNN -> from generator created samples
    fake_decision = D( fake_data )
    error = criterion( fake_decision, torch.ones(seq_len, g_minibatch_size, input_size) )  #  size with g_minibatch_size
    error.backward()
